...
public class SignalProcessingThread extends AsyncTask<SignalProcessingThread.Params, Integer, List<String>>{

    public SignalProcessingResponse<List<Integer>> listener;

    // STRUCT TO SEND PARAMETERS
    public class Params {
        // Variables to calculate peaks
        public double[] data;
        public int samplingFrequency;
        public int windowOverlap;
        public boolean firstProcessing;
        public BufferedWriter denoisedSignalWriter;

        // Variables to calculate HRV
        public List<String> timeSet;
        public BufferedWriter writerPeaksAndHRV;
        int incomingDataCounter;
        int DATA_BLOCK_TO_PROCESS;

        // Interface callback
        public SignalProcessingResponse<List<String>> listener;

        public Params(double[] data, int samplingFrequency, int windowOverlap, boolean firstProcessing, BufferedWriter denoisedSignalWriter,
                      List<String> timeSet, BufferedWriter writerPeaksAndHRV, int incomingDataCounter, int DATA_BLOCK_TO_PROCESS,
                      SignalProcessingResponse<List<String>> listener){
            // For peak detector
            this.data = data.clone();
            this.samplingFrequency = samplingFrequency;
            this.windowOverlap = windowOverlap;
            this.firstProcessing = firstProcessing;
            this.denoisedSignalWriter = denoisedSignalWriter;

            // For HRV
            timeSet = new ArrayList<String>();
            System.arraycopy(timeSet,0,this.timeSet,0,timeSet.size());
            this.writerPeaksAndHRV = writerPeaksAndHRV;
            this.incomingDataCounter = incomingDataCounter;
            this.DATA_BLOCK_TO_PROCESS = DATA_BLOCK_TO_PROCESS;
    ...
    // PROCESSING METHODS
    // Method for test purposes of the algorithm
            ...
            while (line != null)
            {
                ...
                if(valueSetSize == DATA_BLOCK_TO_PROCESS){
                    if(valueSetSize == timeSetSize)
                    {
                        ...
                        List<Integer> validPeakPositions = SignalProcessingThread.ProcessSignals(signalValues, 50, WINDOW_OVERLAP, firstProcessingRound, writerDenoised);
...
//        }
    ...
    */
    ...
    public static List<String> ProcessSignals(Params params) {
        return ProcessSignals(params.data, params.samplingFrequency, params.windowOverlap, params.firstProcessing, params.denoisedSignalWriter,
                                params.timeSet, params.writerPeaksAndHRV, params.incomingDataCounter, params.DATA_BLOCK_TO_PROCESS);
    }


    public static List<String> ProcessSignals(double[] signalToProcess, int samplingFrequency, int windowOverlap, boolean firstProcessing, BufferedWriter denoisedSignalWriter,
                                               List<String> timeSet, BufferedWriter writerPeaksAndHRV, int incomingDataCounter, int DATA_BLOCK_TO_PROCESS) {
        ...
        ////////////// FROM HERE PEAKS ARE CALCULATED

        // First samples were already written
        return CalculateHRV(validPeakPositions, timeSet, writerPeaksAndHRV, incomingDataCounter, DATA_BLOCK_TO_PROCESS);
    }

    private static List<String> CalculateHRV(List<Integer> validPeakPositions, List<String> timeSet, BufferedWriter writerPeaksAndHRV, int incomingDataCounter, int DATA_BLOCK_TO_PROCESS)
    {
        double timeStampPeak = 0.0, timeStampLastPeak = 0.0, calculatedHRV = 0.0;
        String peakTimeStampStr;

        List<String> HRV_strings = new ArrayList<String>();

        //////////////////// LOOP TO PROCESS DETECTED PEAKS
        // The indexes are mapped in the array of timestamps to specify the specific time when the peak was detected
        for(int k=0; k<validPeakPositions.size(); k++) {

            peakTimeStampStr = timeSet.get(validPeakPositions.get(k));
            WriteLog("PEAK " + k + ": " + validPeakPositions.get(k) + " = " + peakTimeStampStr);
            ...
            // From the second peak, the HRV can be calculated properly

            HRV_strings.add("HRV," + timeSet.get(validPeakPositions.get(k)) + "," + Double.toString(calculatedHRV) + "\n");

            try{
                // Write the timestamp corresponding to the detected peak
                writerPeaksAndHRV.write(Integer.toString(incomingDataCounter - DATA_BLOCK_TO_PROCESS + validPeakPositions.get(k)) + "," +
                        timeSet.get(validPeakPositions.get(k)) + "," + Double.toString(calculatedHRV));
    ...
    @Override
    protected List<String> doInBackground(Params... processingParameters) {
        /*
         *
         ...
         */
        return ProcessSignals(processingParameters[0]);
    }
